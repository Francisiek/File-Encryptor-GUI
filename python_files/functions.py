...
def find_file(option):
    ...
    if option==1: var.ENCRYPT_FILE_PATH.set(path)
    elif option==2: var.ENCRYPTION_KEY_PATH.set(path)
    ...
    if var.ENCRYPTION_KEY.get() != "":
        ...
        if file_path:  # jeśli użytkownik nie anulował
            with open(file_path, "wb") as f:
                ...
                var.NEW_ENC_KEY_PATH.set(file_path)
        else:
...
def set_default():
    var.ENCRYPT_FILE_PATH.set("")
    var.ENCRYPTION_KEY_PATH.set("")
    var.NEW_ENC_KEY_PATH.set("")
...
def encrypt():
    encrypt_file_path = var.ENCRYPT_FILE_PATH.get().strip()
    encryption_key_path = var.ENCRYPTION_KEY_PATH.get().strip()
    new_enc_key_path = var.NEW_ENC_KEY_PATH.get().strip()

    if encrypt_file_path != "":
        if encryption_key_path != "" or new_enc_key_path != "":
            if encryption_key_path != "" and  new_enc_key_path != "":
                messagebox.showerror("Error", "Choose ONLY ONE way to encrypt file")
            else:
                key = "" 
                plain_text = ""
                
                with open(encrypt_file_path, 'rb') as origin:
                ...
                if encryption_key_path!="":
                    ...
                    with open(new_enc_key_path, "rb") as keyfile:
                        key=keyfile.read()
                ...
                file_path = filedialog.asksaveasfilename(
                    defaultextension=find_extension(encrypt_file_path),
...
def decrypt():
    ...
    if encrypt_file_path != "":
            if (encryption_key_path != "" and key != ""):
                messagebox.showerror("Error", "Choose ONLY ONE way to decrypt file")
            elif (encryption_key_path == "" and key == ""):
